        ...
        <div className="mt-16 max-w-4xl mx-auto space-y-8">
          {experienceData.map((job, index) => (
            <Card key={index} className="transform transition-transform duration-500 hover:scale-[1.02] bg-card/70 border-l-4 border-primary text-white shadow-lg overflow-hidden border-white/10">
              <CardHeader>
                  <div>
                    <CardTitle className="text-xl font-bold lg:text-2xl text-left">{job.role}</CardTitle>
                    <div className="flex items-center gap-2 mt-1">
                      <Briefcase className="h-5 w-5 text-muted-foreground" />
                      <CardDescription className="text-base font-semibold text-muted-foreground lg:text-lg">{job.company}</CardDescription>
                    </div>
                  </div>
              </CardHeader>
              <CardContent className="text-left">
                <p className="text-muted-foreground text-sm lg:text-base">{job.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
