...
export default function Education() {
  return (
    <AnimatedSection id="education" className="bg-gray-50/70">
        ...
        <div className="relative mt-16 max-w-2xl mx-auto">
          ...
          {educationData.map((item, index) => (
            <div key={index} className={`relative mb-12 flex items-center ${index % 2 === 0 ? 'justify-start text-left' : 'justify-end text-right'}`}>
              <div className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-magenta-600 text-white shadow-lg ring-4 ring-white">
              ...
              <div className={`w-full md:w-5/12 rounded-lg bg-white p-6 shadow-xl transition-all duration-300 hover:shadow-2xl hover:scale-105 border border-gray-200/80 hover:border-magenta-500/50`}>
                  <p className="text-sm font-semibold text-magenta-600">{item.duration}</p>
                  <h3 className="mt-1 text-lg font-bold text-gray-800">{item.institution}</h3>
              </div>
            </div>
          ))}
        </div>
